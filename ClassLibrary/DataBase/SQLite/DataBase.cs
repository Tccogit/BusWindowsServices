...
namespace ClassLibrary
{
    ...
    /// </summary>
    public class JSQLiteDataBase : JCore
    {
        ...
        /// </summary>
        public JSQLiteDataBase(SQLiteConnection pSqlConnectionStringBuilder)
        {
        ...
        public JSQLiteDataBase(string pHost, string pUser, string pPass, string pDataBase)
        {
            ConectionStringBuilder(pHost, pUser, pPass, pDataBase);
            CreateConnection();
        }

        public JSQLiteDataBase(JConnection pCN)
        ...
        /// </summary>
        private SQLiteConnection _cnStrBuilder;
        /// <summary>
        ///  ساخت 
        ///  SqlConnectionString
        ///  مربوط به کانکشن
        /// </summary>
        public SQLiteConnection cnStrBuilder
        {
            get
            {
                CreateConnection();
        ...
        {
            _cnStrBuilder = new SQLiteConnection();
        ...
        private Boolean _Query_DataReader(string SQL, CommandBehavior CB)
        {
            ...
            if (Connect())
            {
                try
                {
                    SQLiteCommand myCommand = new SQLiteCommand(SQL, cn);
                    if (_Transaction != null)
        ...
        {
            SQLiteDataAdapter adapter = new SQLiteDataAdapter(_SQL, cn);
        ...
        private void _SetParams(SQLiteCommand pSqlCommand)
        {
            foreach (KeyValuePair<string, object> Param in Params)
            {
                ...
                pSqlCommand.Parameters.Add(Param.Key, DbType.Object).Value = Value;
            }
        }
        #endregion
        ...
        {
            SQLiteCommand command = new SQLiteCommand(_SQL, cn);
            int recordAffect;
            try
            {
                if (!Connect())
        ...
        {
            object retData = null;
            SQLiteCommand command = new SQLiteCommand(prcName, cn);
            command.CommandType = CommandType.StoredProcedure;
            for (int i = 0; i < parNames.Length; i++)
                command.Parameters.Add(parNames[i], DbType.Object).Value = parValues[i];
            command.Parameters.Add("@ReturnValue", DbType.Int64);
        ...
        public DateTime GetCurrentDateTime()
        {
            ...
            try
            {
                SQLiteCommand command = new SQLiteCommand("select getdate()", cn);
                Connect();
        ...
        private DataTable _Query_DataTable(string pSQL)
        {
            if (Connect())
            {
                SQLiteCommand myCommand = new SQLiteCommand(pSQL, cn);
                try
                {
                    if (_Transaction != null)
                        myCommand.Transaction = _Transaction;
