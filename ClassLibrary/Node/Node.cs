...
namespace ClassLibrary
{
    public class JNode : JSystemNode
    {
        ...
        public JAction RefreshCurrentRowAction;
    ...
    public class JListViewsNodes : JSystemNode
    {
		...
		public JAction AutoRefreshAction ;

		public bool RefreshAuto
		{
			get
			{
				return AutoRefreshAction != null;
			}
		}
		...
		private static int SubPanelWidth = 100;
...
DataTable tblCopy = new DataTable();
        ...
        private void _JanusGrid_MouseClick(object sender, MouseEventArgs e)
        {
            try
            {
                int TempDR = _JanusGrid.Row;
                if (TempDR >=0 && JSystem.Nodes.CurrentRowAction != null)
                {
                    JAction A = new JAction(JSystem.Nodes.CurrentRowAction.Name,
                        JSystem.Nodes.CurrentRowAction.ActionCommand, JSystem.Nodes.CurrentRowAction.Arg,
                        JSystem.Nodes.CurrentRowAction.ConstArg);

                    if (A.Arg == null)
                        A.Arg = new object[] { CurrentNode.Code };
                    else
                    {
                        object[] O = A.Arg;
                        Array.Resize(ref O, O.Length + 1);
                        O[O.Length - 1] = CurrentNode.Code;
                        A.Arg = O;
                    }
                    DataRow DR = (DataRow)A.run();
                    Janus.Windows.GridEX.GridEXRow RX = _JanusGrid.GetRow(TempDR);
                    RX.BeginEdit();
                    foreach (DataColumn dr in DR.Table.Columns)
                    {
                        try
                        {
                            RX.Cells[dr.ColumnName].Value = DR[dr.ColumnName];
                        }
                        catch
                        {
                ...
                if (e.Button == MouseButtons.Right && _JanusGrid.SelectedItems.Count > 0)
                {
                    ...
                    if (ret is JNode)
                    {
                        ...
                        _Node.Row = Row;
                        _Node.Popup.GetPopup(JSystem.Nodes).Show(Cursor.Position);
        ...
        public JAction CurrentRowAction;
        ...
        public JListViewsNodes(TabPage pTabPage, JAction pAction)
        {
            ...
            if (pTabPage != null)
            {
                ...
                {
                    pJanusGrid.ActionClassName = pAction.ActionCommand+"_"+pAction.Name;
                }
				...
				//SubPanel.Height = 0;
                //SubPanel.Width = ReadValueRegister();
				//SubPanel.Resize += SubPanel_Resize;
            }
        }
        ...
        private void SetValueRegister(int SubPanelWidth)
        {
            ...
            key.SetValue("SubPanelWidth", SubPanelWidth);
        ...
        private int ReadValueRegister()
        {
            ...
            return (int)key.GetValue("SubPanelWidth", 25);

        }

        void SubPanel_Resize(object sender, EventArgs e)
		{
            if (SubPanel.Width > 0)
            {
                SubPanelWidth = SubPanel.Width;
                SetValueRegister(SubPanelWidth);
            }
        ...
        void _JanusGrid_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (_JanusGrid.SelectedItems.Count > 0)
            {
                if (e.KeyCode == Keys.Apps)
                    _JanusGrid_MouseClick(sender, new MouseEventArgs(MouseButtons.Right, 1, (_JanusGrid.Width + _JanusGrid.Left) / 2, (_JanusGrid.Width + _JanusGrid.Left) / 2, 0));
            }
        }
        ...
        public void Delete(JNode pNode)
        {
			try
			{
                if (JMainFrame.IsWeb())
                    return;
				if (_JanusGrid == null || _JanusGrid.CurrentRow == null)
					return;
		...
		{
			Refreshdata(Node, pDR, false);
		}
        public void Refreshdata(JNode Node, System.Data.DataRow pDR, bool pAutoRefresh)
        {
            return;
            if (JMainFrame.IsWeb())
                return;
			if (pAutoRefresh)
			{
				try
				{
					pDR = (AutoRefreshAction.run() as DataTable).Rows[0];
				}
				catch
				{
				}
        ...
        public void refreshGrid(bool pRemoveFilters = false, bool pJustSelectRow = false)
        {
            try
            {
                int TempDR = _JanusGrid.Row;
                _JanusGrid.SaveSettings = true;
                if(pRemoveFilters)
                    _JanusGrid.RemoveFilters();
                _JanusGrid.SaveComponentSettings();
                ...
                {
                    if (pJustSelectRow && JSystem.Nodes.CurrentRowAction != null)
                    {
                        JAction A = new JAction(JSystem.Nodes.CurrentRowAction.Name,
                            JSystem.Nodes.CurrentRowAction.ActionCommand, JSystem.Nodes.CurrentRowAction.Arg,
                            JSystem.Nodes.CurrentRowAction.ConstArg);
                ...
                && _ListViewInt.SelectedItems.Count > 0)
            {
                ...
                if (VI.Tag != null)
                {
                    ...
                    if (node.MouseDBClickAction != null && node.MouseDBClickAction.CleareList)
                    {
                        ...
                        CurrentRowAction = node.RefreshCurrentRowAction;
        ...
        private void AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (!DisableSelect && e.Node != null && e.Node.Tag != null)
            {
                ...
                if (_N != null && _N.CurrentAction == null)
                {
                    ...
                    ListViewNodes.CurrentRowAction = CurrentNode.RefreshCurrentRowAction;
