...
{
    public class JOLeDbOpenDataBase
    {
        ...
        int hasnull = 0;
        JOLeDbDataBase[] DBS = new JOLeDbDataBase[0];
        public int add(JOLeDbDataBase pdb)
        {
            try
            {
                if (hasnull > 0)
                {
                    if (DBS.Length - hasnull == 20)
                    {
                        DeleteOlder();
                    }
                ...
                DBS[DBS.Length - 1] = pdb;
                if (DBS.Length > MaxConnection)
                    MaxConnection = DBS.Length;
                return DBS.Length - 1;
            }
            catch (Exception ex)
            {
        ...
        public void delete(int pindex)
        {
            try
            {
                if (pindex < 0 || DBS.Length == 0)
                    return;
                Array.Clear(DBS, pindex, 1);
                hasnull++;
                if (DBS.Length == hasnull)
                {
                    Array.Resize(ref DBS, 0);
                    hasnull = 0;
                }
                if (DBS.Length == 0)
        ...
        public void DeleteOlder()
        {
            ...
            for (int i = 0; i < DBS.Length; i++)
            {
                if (DBS[i] != null && DBS[i].ConnectTime < _dtime)
                {
                    olderIndex = i;
                    _dtime = DBS[i].ConnectTime;
                }
            }
            delete(olderIndex);
        }
        ...
        public void Clear()
        {
            for (int i = 0; i < DBS.Length; i++)
            {
                if (DBS[i] != null && DBS[i].cn.State == ConnectionState.Closed)
                {
                    delete(i);
                }
                else
    ...
    /// </summary>
    public class JOLeDbDataBase : JCore
    {
        ...
        public static JOLeDbOpenDataBase dbsOpen = new JOLeDbOpenDataBase();
        private int OpendbIndex = -1;
        ...
        public JOLeDbDataBase(JConfig pDBConfig)
        {
        ...
        public JOLeDbDataBase(OleDbConnectionStringBuilder pSqlConnectionStringBuilder)
        {
            if (_Config == null)
                _Config = new JConfig();
        ...
        /// </summary>
        private OleDbConnectionStringBuilder _cnStrBuilder;
        /// <summary>
        ///  ساخت 
        ///  SqlConnectionString
        ///  مربوط به کانکشن
        /// </summary>
        public OleDbConnectionStringBuilder cnStrBuilder
        {
            get
            {
                CreateConnection();
        ...
        {
            _cnStrBuilder = new OleDbConnectionStringBuilder();
            _cnStrBuilder.ConnectionString =
            "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=|DataDirectory|\\" + pDBConfig.TimeLogin + ".mdb;Persist " +
            "Security Info=True;Jet OLEDB:Database Password=bascoolpalqoz";
            ...
            //_cnStrBuilder.ConnectTimeout = 30;
        ...
        {
            OleDbDataAdapter adapter = new OleDbDataAdapter(_SQL, cn);
        ...
        private void _SetParams(OleDbCommand pSqlCommand)
        {
        ...
        {
            OleDbCommand command = new OleDbCommand(_SQL, cn);
            int recordAffect;
            try
            {
                if (!Connect())
        ...
        {
            object retData = null;
            OleDbCommand command = new OleDbCommand(prcName, cn);
        ...
        public DateTime GetCurrentDateTime()
        {
            ...
            try
            {
                OleDbCommand command = new OleDbCommand("select getdate()", cn);
                Connect();
        ...
        private DataTable _Query_DataTable(string pSQL)
        {
            if (Connect())
            {
                OleDbCommand myCommand = new OleDbCommand(pSQL, cn);
                try
                {
                    if (_Transaction != null)
                        myCommand.Transaction = _Transaction;
