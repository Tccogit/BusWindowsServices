...
namespace ClassLibrary
{
    public class JStaticNode
    {
        public static JNode _PersonsNode()
        {
            ...
            Node.RefreshCurrentRowAction = new JAction("Persons", "ClassLibrary.JPersons.GetCurrentRow", null, null, true);
        ...
        public static JNode _ShowFormCustomQuery()
        {
            if (!JMainFrame.CurrentUser.IsAdmin)
                return null ;
            JNode Node = new JNode(0, "ClassLibrary.DataBase.CustomQuery.JCustomQueriesForm");
            Node.Name = "اسکیول ساز";

            JAction Ac = new JAction("Form", "ClassLibrary.DataBase.CustomQuery.JCustomQueriesForm.ShowDialog", null, null, true);
            Node.MouseClickAction = Ac;
            Node.MouseDBClickAction = Ac;
